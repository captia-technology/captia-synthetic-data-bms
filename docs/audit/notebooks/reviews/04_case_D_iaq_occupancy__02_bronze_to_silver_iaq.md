# Review — `notebooks/04_case_D_iaq_occupancy/02_bronze_to_silver_iaq.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/04_case_D_iaq_occupancy/02_bronze_to_silver_iaq.ipynb` |
| Título | Caso D · 02 ETL bronce → plata IAQ + poblar captia_metadata |
| Celdas md / code | 23 / 7 |
| Secciones distintas | 22 |
| Outputs persistidos | 6 / 7 (85.7%) |
...
| Datasets detectados | DGT cameras (sintético), Golden set chatbot (sintético), In-Gauge AULA01 (sintético) |

## 1. Resumen ejecutivo

<!-- AUTO -->
Notebook **02_bronze_to_silver_iaq** del caso **IAQ + Occupancy**, etapa **02** (capa Bronce → Plata (ETL)). Score **8.1/10**, veredicto **B**. 6/7 celdas de código con outputs persistidos (85.7%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.
...
- Outputs persistidos celda a celda (85.7%).
