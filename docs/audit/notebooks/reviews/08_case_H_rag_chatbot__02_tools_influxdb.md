# Review — `notebooks/08_case_H_rag_chatbot/02_tools_influxdb.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/08_case_H_rag_chatbot/02_tools_influxdb.ipynb` |
| Título | Caso H · 02 Tools sobre InfluxDB |
| Celdas md / code | 23 / 6 |
...
<!-- AUTO -->
Notebook **02_tools_influxdb** del caso **RAG + Chatbot**, etapa **02** (capa Bronce → Plata (ETL)). Score **8.0/10**, veredicto **B**. 6/6 celdas de código con outputs persistidos (100.0%). Bugs P0/P1 documentados (ver §6). Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.
