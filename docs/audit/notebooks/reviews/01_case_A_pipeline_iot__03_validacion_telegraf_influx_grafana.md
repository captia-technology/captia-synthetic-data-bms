# Review — `notebooks/01_case_A_pipeline_iot/03_validacion_telegraf_influx_grafana.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/01_case_A_pipeline_iot/03_validacion_telegraf_influx_grafana.ipynb` |
| Título | Validación Telegraf → InfluxDB → Grafana |
| Celdas md / code | 23 / 6 |
...
<!-- AUTO -->
Notebook **03_validacion_telegraf_influx_grafana** del caso **Pipeline IoT**, etapa **03** (capa Plata → Oro (Features)). Score **7.6/10**, veredicto **C**. 6/6 celdas de código con outputs persistidos (100.0%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.
