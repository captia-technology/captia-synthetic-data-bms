# Review — `notebooks/10_case_J_traffic_yolo/04_integracion_meteo_trafico.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/10_case_J_traffic_yolo/04_integracion_meteo_trafico.ipynb` |
| Título | Caso J · 04 Predicción congestión a 15 min — tráfico × meteorología |
| Celdas md / code | 23 / 6 |
...
| Datasets detectados | DGT cameras (sintético), Golden set chatbot (sintético) |

## 1. Resumen ejecutivo

<!-- AUTO -->
Notebook **04_integracion_meteo_trafico** del caso **Computer Vision**, etapa **04** (capa Oro (Modelado)). Score **8.6/10**, veredicto **B**. 6/6 celdas de código con outputs persistidos (100.0%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.

## 2. Propósito del notebook

**Caso J · 04 Predicción congestión a 15 min — tráfico × meteorología**.  
Top-5 (8.5/10). Predicción congestión 15min con ablation explícita. `solo_meteo` bate `RF_full`.
...
**Curado:** Sprint 1 fix de P0-4: target lagged `y = congestion_level.shift(-15)`; mock con efecto lluvia (-15% vehicle_count cuando precip>2). DGP mixto: hora+lluvia → señal, NOT vehicle_count.

## 6. Problemas técnicos

- _Sin bugs P0/P1 conocidos._

**Curado:** (resuelto Sprint 1) — original: predecía `Ĉ(t)` no `Ĉ(t+15)`; mock con `corr(vehicle_count, congestion_level) = 0.89` indicaba leakage.

## 7. Problemas didácticos

**Curado:** **vehicle_count introduce ruido si no se normaliza por horario** — insight contraintuitivo bien presentado.
...
Pedag 9 · Código 9 · Rigor 8 · Visu 8 · Ejer 8 · ErrCom 8 · ROI 8 · Reuso 9 · Coher 9 → **8.5**
