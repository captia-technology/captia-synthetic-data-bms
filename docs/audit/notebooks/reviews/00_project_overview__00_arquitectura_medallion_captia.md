# Review — `notebooks/00_project_overview/00_arquitectura_medallion_captia.ipynb`

> **Auditoría:** 2026-05-10  
> **Caso de uso:** Transversal  
> **Etapa:** 00 (Transversal)  
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/00_project_overview/00_arquitectura_medallion_captia.ipynb` |
| Título | Arquitectura Medallion aplicada a CAPTIA Synthetic Data BMS |
| Celdas md / code | 23 / 7 |
...
| Datasets detectados | BDG2 educational (público resampled), DGT cameras (sintético), ERA5 Xàtiva (público mockeado), Golden set chatbot (sintético), LBNL FDD RTU (público mockeado) |

## 1. Resumen ejecutivo

<!-- AUTO -->
Notebook **00_arquitectura_medallion_captia** del caso **Transversal**, etapa **00** (capa Transversal). Score **8.6/10**, veredicto **B**. 7/7 celdas de código con outputs persistidos (100.0%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.
...
- **Dominio:** Transversal.
- **Caso CAPTIA Synthetic Data BMS:** `00_project_overview`.
