# Review — `notebooks/00_project_overview/02_conexion_influxdb_y_variables_entorno.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/00_project_overview/02_conexion_influxdb_y_variables_entorno.ipynb` |
| Título | Conexión a InfluxDB con variables de entorno y `.env` |
| Celdas md / code | 23 / 5 |
...
<!-- AUTO -->
Notebook **02_conexion_influxdb_y_variables_entorno** del caso **Transversal**, etapa **02** (capa Bronce → Plata (ETL)). Score **8.1/10**, veredicto **B**. 5/5 celdas de código con outputs persistidos (100.0%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.
