# Review — `notebooks/03_case_C_hvac_anomaly_detection/05_validacion_fallos_etiquetados.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/03_case_C_hvac_anomaly_detection/05_validacion_fallos_etiquetados.ipynb` |
| Título | Caso C · 05 Validación supervisada con etiquetas — matriz por tipo |
| Celdas md / code | 23 / 6 |
| Secciones distintas | 22 |
| Outputs persistidos | 4 / 6 (66.7%) |
...
| Datasets detectados | LBNL FDD RTU (público mockeado) |

## 1. Resumen ejecutivo

<!-- AUTO -->
Notebook **05_validacion_fallos_etiquetados** del caso **Anomaly Detection**, etapa **05** (capa Oro (Validación)). Score **8.3/10**, veredicto **B**. 4/6 celdas de código con outputs persistidos (66.7%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.
