# Review — `notebooks/02_case_B_energy_forecasting/04_baseline_sarima_xgboost_lstm.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/02_case_B_energy_forecasting/04_baseline_sarima_xgboost_lstm.ipynb` |
| Título | Caso B · 04 Baselines SARIMA / XGBoost / LSTM (opcional) |
| Celdas md / code | 23 / 6 |
...
| Datasets detectados | BDG2 educational (público resampled), Golden set chatbot (sintético) |

## 1. Resumen ejecutivo

<!-- AUTO -->
Notebook **04_baseline_sarima_xgboost_lstm** del caso **Forecasting**, etapa **04** (capa Oro (Modelado)). Score **8.7/10**, veredicto **B**. 6/6 celdas de código con outputs persistidos (100.0%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `eval_helpers`, `plotting`, `synthetic_mocks`.

## 2. Propósito del notebook

**Caso B · 04 Baselines SARIMA / XGBoost / LSTM (opcional)**.  
Compara SARIMA(2,0,2)(1,1,1)_24, XGBoost y `naive_persistence_24h` sobre AULA01 con CV temporal e IC bootstrap 95%.
...
**Curado:** 3 baselines reales (Sprint 3 fix). IC bootstrap. Walk-forward con re-entrenamiento diario.

## 6. Problemas técnicos

- _Sin bugs P0/P1 conocidos._

**Curado:** El notebook NO presenta LSTM aunque el título lo promete (NA-D). Output en mock no muestra significancia clara entre SARIMA y naive_24h.

## 7. Problemas didácticos

**Curado:** Cohesión LaTeX↔código mejorada en Sprint 3. Pendiente: explicar por qué NO LSTM (justificación honesta).
...
Pedag 6 · Código 6 · Rigor 5 · Visu 5 · Ejer 6 · ErrCom 7 · ROI 5 · Reuso 7 · Coher 6 → **6.0**
