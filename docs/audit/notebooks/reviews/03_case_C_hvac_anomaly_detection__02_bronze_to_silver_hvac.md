# Review — `notebooks/03_case_C_hvac_anomaly_detection/02_bronze_to_silver_hvac.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/03_case_C_hvac_anomaly_detection/02_bronze_to_silver_hvac.ipynb` |
| Título | Caso C · 02 ETL bronce → plata HVAC + etiquetas en captia_fault_labels |
| Celdas md / code | 23 / 7 |
...
<!-- AUTO -->
Notebook **02_bronze_to_silver_hvac** del caso **Anomaly Detection**, etapa **02** (capa Bronce → Plata (ETL)). Score **8.1/10**, veredicto **B**. 7/7 celdas de código con outputs persistidos (100.0%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.
