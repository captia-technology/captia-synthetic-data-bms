# Review — `notebooks/07_case_G_data_quality_agents/01_reglas_calidad_bronce.ipynb`
...
| Campo | Valor |
|---|---|
| Ruta | `notebooks/07_case_G_data_quality_agents/01_reglas_calidad_bronce.ipynb` |
| Título | Caso G · 01 Reglas de calidad sobre la capa bronce |
| Celdas md / code | 23 / 6 |
...
| Datasets detectados | BDG2 educational (público resampled), In-Gauge AULA01 (sintético), LBNL FDD RTU (público mockeado) |

## 1. Resumen ejecutivo

<!-- AUTO -->
Notebook **01_reglas_calidad_bronce** del caso **Data Quality + Agents**, etapa **01** (capa Bronce → Plata (EDA)). Score **8.1/10**, veredicto **B**. 6/6 celdas de código con outputs persistidos (100.0%). Sin bugs P0/P1 reportados. Helpers `_common` reutilizados: `captia_schema`, `connection`, `plotting`, `synthetic_mocks`.
