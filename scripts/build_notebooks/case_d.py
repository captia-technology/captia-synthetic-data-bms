...
df.head()
"""),
        section(10, "Exploración paso a paso", "Estadísticas básicas y picos por horario.",
                """\
...
"""),
        ...
        common_summary(next_notebook="04_case_D_iaq_occupancy/02_bronze_to_silver_iaq.ipynb",
                       docs_link="docs/use-cases/case-d-iaq-occupancy.md"),
    ]
    return emit(target=target, rel_path="04_case_D_iaq_occupancy/01_eda_iaq_ocupacion.ipynb",
...
print(mapping)
"""),
        section(10, "Exploración paso a paso", "Para cada columna del CSV producimos una serie "
...
"""),
        ...
        common_summary(next_notebook="04_case_D_iaq_occupancy/03_features_confort_ocupacion.ipynb",
                       docs_link="docs/contracts/variable-catalog.md"),
    ]
    return emit(target=target, rel_path="04_case_D_iaq_occupancy/02_bronze_to_silver_iaq.ipynb",
...
def _features(target: Path) -> Path:
    ...
    sections = [
        ...
        setup_section(),
        section(9, "Carga de datos o mock", "Reusamos el CSV.",
                """\
csv_path = ROOT / "notebooks" / "_data" / "ingauge_aula01_mock.csv"
df = pd.read_csv(csv_path, comment="#", parse_dates=["timestamp"]).set_index("timestamp")
df.head()
"""),
        section(10, "Exploración paso a paso", "Computamos features.",
                """\
def make_features(d):
...
"""),
        ...
        common_summary(next_notebook="04_case_D_iaq_occupancy/04_modelo_ocupacion_desde_ambiente.ipynb",
                       docs_link="docs/use-cases/case-d-iaq-occupancy.md"),
    ]
    return emit(target=target, rel_path="04_case_D_iaq_occupancy/03_features_confort_ocupacion.ipynb",
...
def _model(target: Path) -> Path:
    ...
    sections = [
        ...
        section(3, "Contexto del caso de uso",
                "El modelo se servirá como tool del chatbot del Caso H "
                "(`get_building_state`)."),
        ...
        setup_section(),
        section(9, "Carga de datos o mock", "Cargamos features.",
                """\
...
print(X.shape)
"""),
        section(10, "Exploración paso a paso", "Train/val temporal.",
                """\
y = X.pop("y_occupied")
n = len(X)
i = int(n * 0.7)
...
"""),
        ...
        common_summary(next_notebook="04_case_D_iaq_occupancy/05_validacion_iaq_confort.ipynb",
                       docs_link="docs/validation/ml-validation.md"),
    ]
    return emit(target=target, rel_path="04_case_D_iaq_occupancy/04_modelo_ocupacion_desde_ambiente.ipynb",
                title=title, case=CASE, layer="oro", spec=SPEC, sections=sections)
...
def _val(target: Path) -> Path:
    ...
    sections = [
        ...
        section(3, "Contexto del caso de uso",
                "El profesor recibe alertas cuando CO₂ > 1500 ppm o IAQ > 200 — "
                "señal de ventilar."),
        section(4, "Relación con CENTINELA+",
        ...
        section(7, "Schema CAPTIA esperado", "No aplica."),
        setup_section(),
        section(9, "Carga de datos o mock", "Reusamos In-Gauge.",
                """\
csv_path = ROOT / "notebooks" / "_data" / "ingauge_aula01_mock.csv"
df = pd.read_csv(csv_path, comment="#", parse_dates=["timestamp"]).set_index("timestamp")
df.head()
"""),
        section(10, "Exploración paso a paso", "Categorización CO₂.",
                """\
def cat_co2(x):
...
"""),
        section(11, "Transformación bronce → plata", "No aplica."),
        section(12, "Construcción de capa oro", "Generamos alertas.",
                """\
...
"""),
        ...
        common_summary(next_notebook="05_case_E_weather_solar/01_eda_era5.ipynb",
                       docs_link="docs/use-cases/case-d-iaq-occupancy.md"),
    ]
    return emit(target=target, rel_path="04_case_D_iaq_occupancy/05_validacion_iaq_confort.ipynb",
                title=title, case=CASE, layer="oro", spec=SPEC, sections=sections)
