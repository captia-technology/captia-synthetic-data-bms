...
## Didactic notebooks (45 in `notebooks/`)

Each notebook is generated from a builder under `scripts/build_notebooks/`.
**Do not edit the `.ipynb` files directly** — edit the matching module and
regenerate:

```bash
make notebooks-build       # or `task notebooks:build`
```

The builder is idempotent. Mocks live in `notebooks/_data/` and are
regenerated by `scripts/build_notebook_data.py` (deterministic, `seed=42`).

Helpers shared across notebooks live in `notebooks/_common/` (schema,
connection, mocks, plotting, 18-section template). When adding a new
helper, follow the existing lazy-import pattern (e.g. matplotlib is
imported inside each function so the package is usable in environments
without visual deps).

CI runs `tests/integration/test_notebooks_integrity.py` (183 parametrised
checks): nbformat 4 validity, traceable header (case + medallion + spec),
schema canónico citation, no inline secrets, importability of
`notebooks._common`, deterministic mocks. Run locally with:
