...
def test_legacy_no_ramp_keeps_poisson_jumps() -> None:
    ...
    assert delta >= 10, f"Sin ramp esperamos salto >= 10 personas en transición, got {delta}"
...
def test_ramp_5min_caps_consecutive_delta() -> None:
    ...
    assert p99 <= 3, f"Con ramp=5 min, p99 |Δocc| consecutivo debería ser ≤ 3, got {p99}"
...
def test_ramp_converges_after_5_tau() -> None:
    ...
    assert 17 < mean < 24, f"Tras 5*ramp el output debería estar cerca de 20.4, got mean={mean:.1f}"
